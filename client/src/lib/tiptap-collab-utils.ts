...
// Check if we should use API endpoint for JWT tokens
export const USE_JWT_TOKEN_API_ENDPOINT = 
  process.env.NEXT_PUBLIC_USE_JWT_TOKEN_API_ENDPOINT === "true"

// Validation helpers
export const isConfigValid = () => {
  const hasCollabConfig = TIPTAP_COLLAB_APP_ID && TIPTAP_COLLAB_TOKEN
  const hasAiConfig = TIPTAP_AI_APP_ID && TIPTAP_AI_TOKEN
...
export const fetchCollabToken = async (): Promise<string | null> => {
  ...
  // Use hardcoded token for development
  if (!TIPTAP_COLLAB_TOKEN) {
    const errorMessage = `Collaboration not configured. Please set up your environment variables:
...
- NEXT_PUBLIC_TIPTAP_COLLAB_TOKEN - JWT token for accessing Collaboration services
...
Follow this guide: https://tiptap.dev/docs/ui-components/templates/notion-like-editor`
  ...
  console.warn(
    "Using hardcoded JWT token for development. In production, implement an API endpoint that generates JWT tokens."
  )
...
export const fetchAiToken = async (): Promise<string | null> => {
  ...
  // Use hardcoded token for development
  if (!TIPTAP_AI_TOKEN) {
    const errorMessage = `AI not configured. Please set up your environment variables:
- NEXT_PUBLIC_TIPTAP_AI_APP_ID - Your AI App ID
- NEXT_PUBLIC_TIPTAP_AI_TOKEN - JWT token for accessing AI services
